...
// (2) Write an arrow function that removes a specific book from the bookList
    const remEntry = (title, author) => {
        for(let i = 0; i < bookList.length; i++){
            if(bookList[i].title === title && bookList[i].author === author){
                bookList.splice(i, 1);
                i--; //need to decrements i after removing na element
            }
        }
    }
// (3) Write an arrow function that lists out all the books or all the authors in the book list
    const listAll = (bookList){
        for(let i = 0; i < bookList.length; i++){
            console.log("Book title:" + bookList[i].title + "Book Author: " + bookList[i].author);
        }
    }
